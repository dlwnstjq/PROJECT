...
initInputListeners = () => {
    window.addEventListener('keydown', (e) => {
        ...
        if (e.code === 'Space' || e.code === 'ArrowUp') {
            ...
            // ★ mashBalls 활성 중: 가장 먼저 처리, 다른 판정 코드로 절대 안 넘어감
            if (mashBalls.length > 0) {
                let bestMb = null;
                let bestDist = Infinity;
                for (let mb of mashBalls) {
                    if (mb.gone) continue;         // 완전히 사라진 공만 제외
                    if (mb.speedY <= 0) continue;  // 내려오는 공만 대상 (튕겨 올라가는 중엔 판정 X)
                ...
                if (bestMb) {
                    ...
                    bestMb.speedY = -3;
                    bestMb.bounced = true;  // 튕김 표시 (gone은 아님 — 다시 내려오면 또 판정 가능)
